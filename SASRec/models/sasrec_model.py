...
class SASRecModel(nn.Module):
    def __init__(self, args):
        ...
        self.item_emb = torch.nn.Embedding(self.item_num+2, args.bert_hidden_dim, padding_idx=-1)
        self.pos_emb = torch.nn.Embedding(args.bert_max_len, args.bert_hidden_dim) # TO IMPROVE
        self.emb_dropout = torch.nn.Dropout(p=args.bert_dropout)
        ...
        self.last_layernorm = torch.nn.LayerNorm(args.bert_hidden_dim, eps=1e-8)

        for _ in range(args.bert_num_blocks):
            new_attn_layernorm = torch.nn.LayerNorm(args.bert_hidden_dim, eps=1e-8)
            self.attention_layernorms.append(new_attn_layernorm)

            new_attn_layer =  torch.nn.MultiheadAttention(args.bert_hidden_dim,
                                                            args.bert_num_heads,
                                                            args.bert_dropout)
            self.attention_layers.append(new_attn_layer)

            new_fwd_layernorm = torch.nn.LayerNorm(args.bert_hidden_dim, eps=1e-8)
            self.forward_layernorms.append(new_fwd_layernorm)

            new_fwd_layer = PointWiseFeedForward(args.bert_hidden_dim, args.bert_dropout)
